...
QChart *ThemeWidget::createAreaChartFiliere() const
{
    ...
    QSqlQuery query;
    query.exec("SELECT filliere, COUNT(*) FROM etudiants GROUP BY filliere");
...
QChart *ThemeWidget::createBarChartFiliere() const
{
    ...
    QSqlQuery query("SELECT filliere, COUNT(*) FROM etudiants GROUP BY filliere");
...
//QChart *ThemeWidget::createLineChartFiliere() const
//{
//    dbaccess->open();
//    QSqlDatabase database = dbaccess->database();
...
//        *lineSeries << QPointF(lineSeries->points().count(), count);
//        lineSeries->setName(filiere);
//    }
...
QChart *ThemeWidget::createLineChartFiliere() const
{
    ...
    int x = 0; // Counter variable for x-coordinate
    while (query.next()) {
        QString filiereName = query.value(0).toString();
        int studentCount = query.value(1).toInt();
        series->append(x, studentCount);
        series->setName(filiereName);
        x++;
    }
...
QChart* ThemeWidget::createSplineChartFiliere() const
{
    dbaccess->open();
    QSqlDatabase database = dbaccess->database();
    ...
    chart->setAxisX(axisX, splineSeries); // Add X-axis to the chart

    while (query.next()) {
        QString filiere = query.value(0).toString();
        int count = query.value(1).toInt();

        QPointF point(filiere.toDouble(), count);
...
QChart *ThemeWidget::createAreaChartSexe() const
{
    ...
    QSqlQuery query;
    query.exec("SELECT genre, COUNT(*) FROM etudiants GROUP BY genre");
...
QChart *ThemeWidget::createBarChartSexe() const
{
    ...
    QSqlQuery query("SELECT genre, COUNT(*) FROM etudiants GROUP BY genre");
    QBarSet *barSet = new QBarSet("");
    QBarSeries *barSeries = new QBarSeries();
...
//        chartView = new QChartView(createLineChartFiliere());
//        // Funny things happen if the pie slice labels do not fit the screen, so we ignore size policy
//        //chartView->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
...
//                 chartView = new QChartView(createSplineChartFiliere());
//                // Funny things happen if the pie slice labels do not fit the screen, so we ignore size policy
//                chartView->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
//                m_ui->gridLayout->addWidget(chartView, 0, 0);
//                m_charts << chartView;
        chartView = new QChartView(createAreaChartClass());
        m_ui->gridLayout->addWidget(chartView, 2, 1);
        m_charts << chartView;
